...
const peopleAge = persons.reduce((acc, person) => {
...
const frenchPeople = persons.filter((person) => {
  return person.country === 'FR';
});

const frenchPeopleAge = frenchPeople.reduce((acc, person) => {
  return (acc += person.age);
}, 0);

console.log(frenchPeopleAge);

//  *  - 3. Obtén un array con el nombre de todas las mascotas.

const arrayPets = persons.map((person) => person.pet.name);
...
console.log(personsWithCat);

//  *  - 5. Obtén un array con los coches de los españoles.
const spanishWithCar = persons.filter((person) => {
  return person.country === 'ES' && cars.find((car) => person.car);
});

console.log(spanishWithCar);

const spanishCopy = { ...spanishWithCar };

const onlyCars = spanishCopy.map((person) => person.car);
